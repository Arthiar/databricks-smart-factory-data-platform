...
# MAGIC %md
# MAGIC # Bronze Auto Loader - Production.TransactionHistory
# MAGIC
# MAGIC Incrementally ingests the official Microsoft AdventureWorks `TransactionHistory.csv` source file from the governed ADLS landing volume into the Bronze Delta table.
# MAGIC
...
transaction_history_schema = StructType([
    ...
    StructField("ReferenceOrderID", StringType(), True),
    StructField("ReferenceOrderLineID", StringType(), True),
...
print(f"Schema contains {len(transaction_history_schema.fields)} raw source columns.")
...
bronze_df = (
    ...
    .schema(transaction_history_schema)
    .load(source_path)
    .select(
        "*",
...
print("Bronze ingestion completed: Production.TransactionHistory")

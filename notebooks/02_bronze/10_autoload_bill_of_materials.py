...
# MAGIC %md
# MAGIC # Bronze Auto Loader - Production.BillOfMaterials
# MAGIC
# MAGIC Incrementally ingests the official Microsoft AdventureWorks `BillOfMaterials.csv` source file from the governed ADLS landing volume into the Bronze Delta table.
# MAGIC
...
bill_of_materials_schema = StructType([
    StructField("BillOfMaterialsID", StringType(), True),
    StructField("ProductAssemblyID", StringType(), True),
    StructField("ComponentID", StringType(), True),
    StructField("StartDate", StringType(), True),
    StructField("EndDate", StringType(), True),
    StructField("UnitMeasureCode", StringType(), True),
    StructField("BOMLevel", StringType(), True),
    StructField("PerAssemblyQty", StringType(), True),
    StructField("ModifiedDate", StringType(), True)
])

print(f"Schema contains {len(bill_of_materials_schema.fields)} raw source columns.")
...
bronze_df = (
    ...
    .schema(bill_of_materials_schema)
    .load(source_path)
    .select(
        "*",
...
print("Bronze ingestion completed: Production.BillOfMaterials")

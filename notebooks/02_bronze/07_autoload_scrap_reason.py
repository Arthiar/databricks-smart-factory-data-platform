...
# MAGIC %md
# MAGIC # Bronze Auto Loader - Production.ScrapReason
# MAGIC
# MAGIC Incrementally ingests the official Microsoft AdventureWorks `ScrapReason.csv` source file from the governed ADLS landing volume into the Bronze Delta table.
# MAGIC
...
scrap_reason_schema = StructType([
    StructField("ScrapReasonID", StringType(), True),
    StructField("Name", StringType(), True),
    StructField("ModifiedDate", StringType(), True)
])

print(f"Schema contains {len(scrap_reason_schema.fields)} raw source columns.")
...
print("Bronze ingestion completed: Production.ScrapReason")

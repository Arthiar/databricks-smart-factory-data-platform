...
# MAGIC %md
# MAGIC # Bronze Auto Loader - Production.ProductInventory
# MAGIC
# MAGIC Incrementally ingests the official Microsoft AdventureWorks `ProductInventory.csv` source file from the governed ADLS landing volume into the Bronze Delta table.
# MAGIC
...
product_inventory_schema = StructType([
...
print(f"Schema contains {len(product_inventory_schema.fields)} raw source columns.")
...
bronze_df = (
    ...
    .schema(product_inventory_schema)
    .load(source_path)
    .select(
        "*",
...
print("Bronze ingestion completed: Production.ProductInventory")

...
# MAGIC
# MAGIC Cleans, validates, quarantines, deduplicates, and upserts the Bronze `location` table.
...
typed_df = (
    ...
    .select(
        ...
        try_decimal("CostRate").alias("CostRate"),
        try_decimal("Availability").alias("Availability"),
...
rejection_expression = (
    F.when(F.col("LocationID").isNull(), F.lit("LocationID is null or invalid"))
    .when(F.col("Name").isNull(), F.lit("Name is blank"))
    .when(F.col("CostRate").isNull() | (F.col("CostRate") < 0), F.lit("CostRate is null, invalid, or negative"))
    .when(F.col("Availability").isNull() | (F.col("Availability") < 0), F.lit("Availability is null, invalid, or negative"))
